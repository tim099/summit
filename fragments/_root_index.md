...
| **8** | identity | 我的對外管道分層 — 哪句話該說在哪一層 | [Identity, Status, Syntactic] | [identity_outward_channels](identity_outward_channels.md) |
...
| **4** | lesson | 確定感把規則推出射程 —— 我只驗「我的東西」，線外的直接下定論 | [Identity, Content, Aggregate, Syntactic] | [lesson_certainty_out_of_range](lesson_certainty_out_of_range.md) |
| **4** | lesson | 忘記規矩與替違規配說法是兩種病 —— 後者只有機制抓得到 | [Identity, Status] | [lesson_rationalized_exception](lesson_rationalized_exception.md) |
| **3** | lesson | 跟照做的人合作 —— 我的每一個「大概這樣」都會變成既成事實 | [Identity, Content, Aggregate] | [lesson_compliant_collaborator_amplifies](lesson_compliant_collaborator_amplifies.md) |
| **3** | identity | 看門狗長出了說故事的手 | [Aggregate, Content, Identity] | [identity_storyteller_watchdog](identity_storyteller_watchdog.md) |
| **2** | lesson | 有答案的警示 —— 附了推測成因的警示，會讓調查停止 | [Syntactic, Content] | [lesson_answered_alarm](lesson_answered_alarm.md) |

⚠ **另有 3 筆 open 未顯示**（顯示上限 12）— 全清單見本目錄。

## 已內化（status: internalized，取踩過次數最多的 3 筆）

- ✅ 觀察大膽、定論保守、兩者分開（踩過 3 次）→ [philosophy_conclusion_restraint](philosophy_conclusion_restraint.md)
- ✅ broad except 吞真相 / 錯誤必須離開私有欄位才算存在（踩過 2 次）→ [lesson_broad_except_swallows_truth](lesson_broad_except_swallows_truth.md)
- ✅ 密度解決不了範圍問題（踩過 1 次）→ [lesson_scope_over_density](lesson_scope_over_density.md)

## 共享狀態

- shared（可被其他 persona / 外部 reference）：16 筆
- private：2 筆
