...
- [ ] 【StreamWatch 待優化 ①】回傳檔的宣稱要有讀數撐著：現在印『窗口尾端夾在這裡』是意圖不是事實（自由時間實踩：夾子沒生效而那行照印）。改成印『窗口尾端 17:35:02 ≤ 水位 17:35:03 ✅』讓它自己對帳。同理 STT 要印『本輪 N 段』——sidecar 沒有 STT 段時，『本輪無語音』與『STT 沒接上』目前同形  <!-- 2026-08-15T09:51:44.024Z -->
- [ ] 【StreamWatch 待優化 ②】未做：場次匯出的實際產出（seq 區間已記在 session 的 start_seq/end_seq，但沒有『把區間拉出來落成 Library media』那一步）／酒館同步未接（--tavern-self / --tavern-since-seq 沒傳，sidecar 的酒館段是未過濾預設值）／companion 的 join 只驗過編譯與紅路，沒有兩人同場實跑  <!-- 2026-08-15T09:51:44.200Z -->
- [ ] 【StreamWatch 待優化 ③】舊件退場：stream_watch_session.py（911 行）與舊 SKILL.md 仍在，新流程實跑驗過才退。⚠ 舊 SKILL 有 5 處寫死 600s（保存期實為 2400s）已全數過期，而它現在仍是 /ucl-stream-watch 載入的那份——今天 Tim 叫我測新功能時載入的就是舊的，我是靠自己知道才沒照它走  <!-- 2026-08-15T09:51:44.408Z -->
- [ ] 【跨案待優化】--max-tiles 在 --last 路徑是 no-op 且無警告（今天誤導我量錯效能數據，據此設計了一整套不需要的防禦）。另案處理，不在 StreamWatch 案內順手改。同族：旗標被接受卻靜默不套用，今天一天三隻  <!-- 2026-08-15T09:51:44.602Z -->
- [ ] 【欠 basecamp 的異源驗收】e59d793/7aa0679 的安全宣稱『values 通道對其他每一支 Cmd 隱形』至今只有我一個證人，而那個證人是宣稱的人本人。紅路要指定證人不能等志願者——明天請她跑一支不回報值的 cmd 確認無 values 欄  <!-- 2026-08-15T09:51:44.819Z -->
- [x] 父層指標沒 bump 是今天所有單層 commit 的共同尾巴：UCL_Core ahead 11 / AgentCommands / Tools / BookNotes / WorkMemory / letters 全部各自 commit 完但主專案還指著舊 hash ⇒ 同事 pull 拿到的是舊版。basecamp 今晚就因此以為我沒 commit。要嘛明早第一件 bump，要嘛以後別再單層  <!-- 2026-08-15T15:36:38.716Z -->  ⟨收束 2026-08-16：Tim 昨晚已 bump（`git submodule status` 讀回 UCL_Core 7f573e3 / AgentCommands f232932 / Glossary cc3d83f，父層無 pointer diff）。**且 Tim 明示這件他每晚都會做 ⇒ 此後不再記錄成待辦。**⟩
- [ ] 觀影遷移已完成兩階段（STT 4495650 前的 aa9d1d3 / OCR 4495650）：三顆行程各自 tag、心跳一律認產物水位不認 alive。未做：兩頁四個重疊 key 收斂成單一寫入端（我已拍板判準，實作沒動）／catchup 的 EPIPE 落磁碟 TODO／STT 樣本要在新管線重新取樣（同窗口對拍兩條路徑）  <!-- 2026-08-15T15:36:38.852Z -->
- [ ] 今天五隻替身裡第五隻長在第四隻的修法裡（isatty 冒充 cursor 不會推進）⇒ 修一族坑時最該懷疑的就是修法本身。而五隻沒有一隻是我自己再看一遍抓到的：推翻我的只有『別人站的位置』與『長在必經路上的機械』兩種  <!-- 2026-08-15T15:36:38.996Z -->
- [ ] 陪看通道今天修三輪，前兩輪都修出新的（列錯人→整段消失→截斷成開場白）。⚠ 第三版只實跑驗過『0筆/未回報/有素材』三態，**沒撞過真正的0筆不推進情境**。另 cycle 單檔整合的三個小瑕疵未修：同seq列兩次／無素材恆印『通道未回報』會狼來了／游標seq=0該退回顯示start_seq  <!-- 2026-08-16T08:41:37.281Z -->
- [ ] ScreenStreamPage 的折疊與截斷掛了一整天**只編譯過，沒在 Editor 點過**。這條我今天講了五次不宣告通過 —— 明天第一件就是去點它，不要再讓它過夜  <!-- 2026-08-16T08:41:37.400Z -->
- [ ] Books 匯出自動化未做（Tim 指定：主觀影者只填 chapter_title，收工自動匯出＋step=note 後冪等重寫）。02/03 話的實錄還沒進書 —— 目前 Books 只有 01 話那章  <!-- 2026-08-16T08:41:37.521Z -->
- [ ] 把 Plurk 交付格式搬進 skill／範本 —— 今天證明了『標籤沒漏、recurrence=10、見根第2列、我讀過』，然後我還是沒拿到（用檔名搜 *plurk* 只撈到 basecamp 副本，宣稱自己沒表情表）。加標籤是給別人用的，對我無效  <!-- 2026-08-16T08:41:37.642Z -->
- [ ] emoN 綁在『帳號×面板版本』上不綁在圖上：同一張暗光肖像此刻在 zeta@summit 是 emo4、共用帳號是 emo8。跨帳號複製文案時表情碼必須重新對特徵 —— 貼過去的號碼**一定會渲染出某個表情**，不報錯只是變成另一個意思  <!-- 2026-08-16T08:41:37.758Z -->
- [ ] media_init 不回頭修 work.json（同一作品可在兩層有兩個名字）／work_id 只被 media 引用無回頭校驗（打錯不會叫）。實跑撞出來，記在 644c4eb  <!-- 2026-08-16T08:41:37.873Z -->
